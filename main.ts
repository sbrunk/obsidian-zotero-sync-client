...
// Authors
if (data.creators) {
	frontmatter.author = data.creators.map(author => 
...
// Publication date
if (data.date) {
	frontmatter.published = data.date;
}

// Added date
if (data.dateAdded) {
	frontmatter.created = data.dateAdded;
}
...
export default class ZoteroSyncClientPlugin extends Plugin {
	...
	async applyUpdates(library: ZoteroRemoteLibrary) {
		...
		const renames: {
			[key: string]: {
				from: string, to: string, note: string, zoteroKey: string, frontmatter: Record<string, any>, element: ZoteroCollectionItem | ZoteroItem
			}
		} = {};
		const updates: {
			[key: string]: {
				filePath: string, note: string, zoteroKey: string, frontmatter: Record<string, any>, element: ZoteroCollectionItem | ZoteroItem
			}
		} = {};
		...
		const computeChanges = (element: ZoteroCollectionItem | ZoteroItem, status: Map<string, ZoteroNoteStatus>, updatedStatus: Map<string, ZoteroNoteStatus>) => {
			...
			if (status.get(key)) {
				...
				if (status.get(key)?.filePath != filePath) {
					// rename
					renames[key] = { from: status.get(key)?.filePath || '', to: filePath, note: note, zoteroKey: element.key, frontmatter: frontmatter, element: element }
				}
				// does it need to be updated?
				if (status.get(key)?.hash !== hash) {
					// update
					updates[key] = { filePath: filePath, note: note, zoteroKey: element.key, frontmatter: frontmatter, element: element }
				}
				...
				// create
				creates[key] = { filePath: filePath, note: note, zoteroKey: element.key, frontmatter: frontmatter, element: element }
			}
		...
		for (const [key, value] of Object.entries(renames)) {
			try {
				await this.renameFile(value.from, value.to, value.note, value.zoteroKey, library, value.element, value.frontmatter)
			} catch (e) {
		...
		for (const [key, value] of Object.entries(updates)) {
			try {
				await this.updateFile(value.filePath, value.note, value.zoteroKey, library, value.element, value.frontmatter)
			} catch (e) {
		...
		for (const [key, value] of Object.entries(creates)) {
			try {
				await this.createFile(value.filePath, value.note, value.zoteroKey, library, value.element, value.frontmatter)
			} catch (e) {
	...
	aquireFile(filePath: string): TAbstractFile | null {
		...
		//       (e.g. by checking for the frontmatter zotero_key property using hasZoteroFrontmatter)
		return this.app.vault.getAbstractFileByPath(filePath)
	}

	async isZoteroManagedFile(file: TFile): Promise<boolean> {
		// Check if file is managed by this plugin by checking for zotero frontmatter
		return await this.hasZoteroFrontmatter(file);
	}
	...
	async renameFile(oldPath: string, newPath: string, note: string, zoteroKey: string, library: ZoteroRemoteLibrary, element: ZoteroCollectionItem | ZoteroItem, frontmatter: Record<string, any> = {}) {
		const fn = this.aquireFile(oldPath)
		await this.ensureDirectoryExists(newPath)
		if (!fn) {
			await this.createFile(newPath, note, zoteroKey, library, element, frontmatter)
		} else {
			...
			if (newFile) {
				...
				await this.setZoteroFrontmatter(newFile, zoteroKey, library, element, frontmatter);
			}
		}
	}

	async updateFile(filePath: string, note: string, zoteroKey: string, library: ZoteroRemoteLibrary, element: ZoteroCollectionItem | ZoteroItem, frontmatter: Record<string, any> = {}) {
		const fn = this.aquireFile(filePath)
		if (!fn) {
			await this.createFile(filePath, note, zoteroKey, library, element, frontmatter)
		} else {
			await this.app.vault.modify(fn as TFile, note)
			await this.setZoteroFrontmatter(fn as TFile, zoteroKey, library, element, frontmatter)
		}
	}

	async createFile(filePath: string, note: string, zoteroKey: string, library: ZoteroRemoteLibrary, element: ZoteroCollectionItem | ZoteroItem, frontmatter: Record<string, any> = {}) {
		const fn = this.aquireFile(filePath)
		if (fn) {
			await this.app.vault.modify(fn as TFile, note)
			await this.setZoteroFrontmatter(fn as TFile, zoteroKey, library, element, frontmatter)
	...
	generateZoteroLink(element: ZoteroCollectionItem | ZoteroItem, library: ZoteroRemoteLibrary): string {
		// Generate Zotero link like the old getMarker function
	...
	async setZoteroFrontmatter(file: TFile, zoteroKey: string, library: ZoteroRemoteLibrary, element: ZoteroCollectionItem | ZoteroItem, additionalFrontmatter: Record<string, any> = {}): Promise<void> {
		// Use Obsidian API to set the zotero frontmatter properties and additional properties
		await this.app.fileManager.processFrontMatter(file, (frontmatter) => {
			// Set the zotero key for identification
			frontmatter.zotero_key = zoteroKey;
			
			// Set the zotero link for easy access
			frontmatter.zotero = this.generateZoteroLink(element, library);
	...
	async hasZoteroFrontmatter(file: TFile): Promise<boolean> {
		// Check if file has zotero_key property in frontmatter
		const metadata = this.app.metadataCache.getFileCache(file);
		return metadata?.frontmatter?.zotero_key !== undefined;
	}

	async getZoteroKey(file: TFile): Promise<string | null> {
		// Get the zotero key from frontmatter
		const metadata = this.app.metadataCache.getFileCache(file);
		return metadata?.frontmatter?.zotero_key || null;
	}
			...
			const refreshPreview = async () => {
				...
				if (previewType === 'md') {
					try {
						...
						let previewNote = '---\n';
						previewNote += `zotero_key: ${element.key}\n`;
						previewNote += `zotero: ${this.plugin.generateZoteroLink(element, library)}\n`;
